...
class GPO2(nn.Module):
    def __init__(self, d_pe, d_hidden):
        super(GPO2, self).__init__()
        self.d_pe = d_pe
        self.d_hidden = d_hidden
        ...
        self.gru = nn.GRU(self.d_pe, d_hidden, 1, batch_first=True, bidirectional=True)
        self.linear = nn.Linear(self.d_hidden, 1, bias=False)
        self.itr_module = InteractionModule(self.d_pe, self.d_hidden,3,4,128)
    ...
    def forward(self, features, lengths):
        ...
        pooled_features = self.itr_module(pooled_features,True)
